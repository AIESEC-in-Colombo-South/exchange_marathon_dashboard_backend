...
import type { CriteriaCounts, MemberProfile, MemberSnapshot, SyncSource } from "./types.js";
...
function normalizeTimestamp(value: string, fallbackIso: string): string {
 ...
 */
async function getActiveSources(): Promise<SyncSource[]> {
...
export async function runSync(): Promise<SyncResult> {
  ...
  const functionsSet = new Map<string, { slug: string; name: string }>();
  const teamsSet = new Map<string, { slug: string; name: string; functionSlug: string }>();
  const memberAggregator = new Map<string, any>();
    ...
    try {
      ...
      for (const [sheetName, rows] of Object.entries(multiSheetData)) {
        ...
        for (const row of rows) {
          ...
          if (!acc) {
            const profile: MemberProfile = { email, name, role, functionSlug, team: teamSlug };
            acc = {
              profile,
          ...
          if (sourceTimestamp > acc.sourceUpdatedAt) acc.sourceUpdatedAt = sourceTimestamp;
        }
      }
  ...
  if (functionsData.length > 0) await supabase.from("functions").upsert(functionsData);
  ...
  if (teamsData.length > 0) await supabase.from("teams").upsert(teamsData);
  ...
  if (membersToUpsert.length > 0) {
    ...
    if (membersError) console.error("❌ Members upsert failed:", membersError.message);
  }
  ...
  const periods: ("daily" | "weekly" | "marathon")[] = ["daily", "weekly", "marathon"];
  const allDailySnapshots: MemberSnapshot[] = snapshotsToUpsert.map(s => {
  ...
  const cachePromises = Array.from(teamsSet.values()).flatMap(team => 
    periods.map(async period => {
      try {
        const dashboard = await getTeamDashboard(team.slug, period as any, undefined, allDailySnapshots);
  ...
  const functionCachePromises = Array.from(functionsSet.values()).flatMap(func => 
    periods.map(async period => {
      try {
        const dashboard = await getTeamDashboard(func.slug, period as any, undefined, allDailySnapshots, "function");
        dashboard.syncInfo = { lastSyncTime: syncStart, nextSyncTime: nowIso(), intervalMinutes: config.syncScheduler.intervalMinutes, runId };
        await supabase.from("dashboard_cache").upsert({
